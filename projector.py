    ...
    def set_network(self, Gs, dtype='float16'):
        ...
        if sh[2] > 256:
            ...
            proc_images_expr = tf.reduce_mean(
                tf.reshape(proc_images_expr, [-1, sh[1], sh[2] // factor, factor, sh[2] // factor, factor]),
                axis=[3, 5])
    ...
    def start(self, target_images):
        ...
        if sh[2] > self._target_images_var.shape[2]:
            ...
            target_images = np.reshape(target_images,
                                       [-1, sh[1], sh[2] // factor, factor, sh[3] // factor, factor]).mean((3, 5))
...
def project(network_pkl: str, target_fname: str, outdir: str, save_video: bool, seed: int):
    ...
    # Save results.
    img = PIL.Image.fromarray(proj.images_uint8[0], 'RGB')
    img.save(f'{outdir}/proj.png')
    display(img)
    display(img)
...
def project2(network_pkl: str, b64: str, outdir: str, save_video: bool, seed: int):
    # Load networks.
    tflib.init_tf({'rnd.np_random_seed': seed})
    print('Loading networks from "%s"...' % network_pkl)
    with dnnlib.util.open_url(network_pkl) as fp:
    ...
    # Load target image.
    target_pil = base64_to_image(b64)
    w, h = target_pil.size
    s = min(w, h)
    target_pil = target_pil.crop(((w - s) // 2, (h - s) // 2, (w + s) // 2, (h + s) // 2))
    target_pil = target_pil.convert('RGB')
