...
def style_mixing_example2(network_pkl, truncation_psi, col_styles, outdir, minibatch_size=4):
    tflib.init_tf()
    print('Loading networks from "%s"...' % network_pkl)
    with dnnlib.util.open_url(network_pkl) as fp:
        _G, _D, Gs = pickle.load(fp)

    dlat1 = np.load('./dlatents1.npz')
    dlat2 = np.load('./dlatents3.npz')
    print(dlat1.f.dlatents)
    np1 = dlat1.f.dlatents
    np2 = dlat2.f.dlatents
    ...
    print(type(np1))
    print(np1.shape)
    all_w = np.concatenate((np1, np2), 0)
    # all_w[0] = np1
    # all_w[1] = np2
    # all_w = w_avg + (all_w - w_avg) * truncation_psi # [minibatch, layer, component]
    w_dict = {seed: w for seed, w in zip(all_seeds, list(all_w))} # [layer, component]
...
def main():
    ...
    style_mixing_example2(**vars(args))
